...
def run_simulation_async(simulation_id):
    ...
    # FIX: Initialize simulation to None to prevent unbound errors
    simulation = None
    with app.app_context():
        try:
            simulation = Simulation.query.get(simulation_id)
            if not simulation:
                # Log or handle the case where the simulation ID is invalid
            ...
            # FIX: Check if simulation object exists before trying to update it
            if simulation:
...
def run_seir_simulation(simulation, params):
    ...
    initial_conditions = params.get(
        "initial_conditions",
        {"S": params.get("population", 100000) - 1, "E": 0, "I": 1, "R": 0},
    )
...
def calculate_simulation_metrics(results, model_type):
    ...
    if model_type in ["seir", "agent_based", "network"]:
        if "infectious" in results:
            ...
            metrics["max_infections"] = (
                float(np.max(infectious)) if infectious.size > 0 else 0
            )
            metrics["total_infections"] = (
                float(np.sum(infectious)) if infectious.size > 0 else 0
            )
            metrics["peak_day"] = (
                int(np.argmax(infectious)) if infectious.size > 0 else 0
            )

        if "recovered" in results:
            recovered = np.array(results["recovered"])
            metrics["final_recovered"] = (
                float(recovered[-1]) if recovered.size > 0 else 0
            )
...
def create_simulation():
    ...
    try:
        ...
        if dataset_id and not PermissionManager.can_access_dataset(user, dataset_id):
        ...
        audit_log.set_details(
            {"simulation_name": simulation.name, "model_type": simulation.model_type}
        )
...
def compare_simulations():
    ...
    try:
        ...
        if len(simulation_ids) < 2:
            return (
                jsonify({"error": "At least 2 simulations required for comparison"}),
                400,
            )
        ...
        comparative_metrics = {}
        peak_infections = {
            sim_id: data["metrics"].get("max_infections")
            for sim_id, data in comparison_results.items()
            if "max_infections" in data.get("metrics", {})
        }

        if peak_infections:
            comparative_metrics["peak_infections"] = peak_infections
            # FIX: Use a lambda function for the key argument in max/min
            comparative_metrics["highest_peak"] = max(
                peak_infections, key=lambda k: peak_infections[k]
            )
        ...
        r0_values = {
            sim_id: data["metrics"].get("r0")
            for sim_id, data in comparison_results.items()
            if "r0" in data.get("metrics", {})
        }
